...
class Elementor_Post_Grid_Widget extends Widget_Base {
    ...
    private function wpcap_content_layout_options() {
        ...
        $this->add_control(
            'grid_style', [
                ...
                'default' => '1',
                'options' => [
                    '1' => esc_html__('Classic', 'post-grid-elementor-addon'),
                    '2' => esc_html__('Modern', 'post-grid-elementor-addon'),
                    '3' => esc_html__('Listing', 'post-grid-elementor-addon'),
        ...
        $this->add_control(
            'filter_fields', [
                'label' => __('Filter Fields', 'post-grid-elementor-addon'),
                'label_block' => true,
                'type' => Controls_Manager::SELECT2,
                'default' => ['region', 'categories'],
                'multiple' => true,
                'options' => [
                    'categories' => __('Categories', 'post-grid-elementor-addon'),
                    'company' => __('Company', 'post-grid-elementor-addon'),
                    'region' => __('Region', 'post-grid-elementor-addon'),
                ],
                'separator' => 'before',
                'condition' => [
                    'display_filters' => 'yes',
    ...
    protected function render($instance = []) {
        ...
        <div class="wpcap-grid">
            <?php $this->render_filters($query_args); ?>
            <?php
            ...
            ?>
            <div class="display_filter_data wpcap-grid-container elementor-grid <?php echo $columns_desktop . $columns_tablet . $columns_mobile . $grid_class; ?>">
    ...
    private function get_filter_field($fieldname) {
        switch ($fieldname) {
            case 'categories': ?>
                <?php 
                    $post_tax = $this->get_post_taxonomy( 'cpt_posts', $this->get_settings() );
                    $categories = get_categories(['taxonomy' => $post_tax, 'hide_empty' => false]); 
                    $filters_categories = isset($_GET['categories']) ? $_GET['categories'] : [];
                ?>
                <div class="ToggleButton">
                    <button class="selectbutton cpt_filter_option" data-id="cpt_filter_categories"><?php _e('Select Type(s)', 'wp-cpt'); ?>
                        <img src="<?php echo CPT_URL; ?>/assets/img/selectimg02.png" alt="Img">
                    </button>
                    <div id="cpt_filter_categories" class="SelectBox cpt_filter_box">
                        <?php foreach($categories as $key => $category): ?>
                            <span class="dropdowntitle">
                                <input <?php echo in_array($category->term_id, $filters_categories) ? 'checked' : ''; ?> value="<?php echo $category->term_id; ?>" type="checkbox" name="categories[]" class="checkbox cpt_filter_checkbox">
            ...
            <?php break;
            case 'company':
                $choices = get_field_object('field_5ccbd99f08a16'); 
                $filters_companies = isset($_GET['company']) ? $_GET['company'] : [];
                ?>
                <div class="ToggleButton">
                    <button class="selectbutton cpt_filter_option" data-id="cpt_filter_company"><?php _e('Filter By Company', 'wp-cpt'); ?>
                        <img src="<?php echo CPT_URL; ?>/assets/img/selectimg02.png" alt="Img">
                    </button>
                    <div id="cpt_filter_company" class="SelectBox cpt_filter_box">
                        <?php foreach($choices['choices'] as $fieldkey => $fieldlabel): ?>
                            <span class="dropdowntitle">
                                <input <?php echo in_array($fieldkey, $filters_companies) ? 'checked' : ''; ?> type="checkbox" name="company[]" value="<?php echo $fieldkey; ?>" class="checkbox cpt_filter_checkbox">
                                <span><?php echo $fieldlabel; ?> </span>
                            </span>
                        <?php endforeach; ?>
                    </div>
                </div>
            <?php break;
            case 'region':
                $filters_regions = isset($_GET['regions']) ? $_GET['regions'] : [];
                ?>
                <div class="ToggleButton">
                    <button class="selectbutton cpt_filter_option" data-id="cpt_filter_region"><?php _e('Filter By Region', 'wp-cpt'); ?>
                        <img src="<?php echo CPT_URL; ?>/assets/img/selectimg02.png" alt="Img">
                    </button>
                    <div id="cpt_filter_region" class="SelectBox cpt_filter_box">
                        <?php foreach(cpt()->get_regions() as $key => $region): $regionslug = get_post_meta($region->ID, 'region_subdomain', true); ?>
                            <span class="dropdowntitle">
                                <input <?php echo WP_REGION == $regionslug ? 'checked' : ''; ?> <?php echo in_array($regionslug, $filters_regions) ? 'checked' : ''; ?> type="checkbox" value="<?php echo $regionslug; ?>" name="regions[]" class="checkbox cpt_filter_checkbox">
                                <span><?php echo $region->post_title; ?></span>
                            </span>
                        <?php endforeach; ?>
                    </div>
                </div>
    ...
    protected function render_filters($query_args) {
        $settings = $this->get_settings();
        $display_filters = $settings['display_filters'];
        if ('yes' !== $display_filters) {
            return;
        }

        $filter_fields = $settings['filter_fields']; 
        ?>
        <div class="cpt_filter_wrap">
            <form class="cpt_filter_form" id="cpt_filter_form" method="get">
                <input type="hidden" name="action" value="cpt_filter">
                <input type="hidden" name="taxonomy" value="<?php echo $this->get_post_taxonomy( 'cpt_posts', $this->get_settings() ); ?>">
                <input type="hidden" name="region" value="<?php echo WP_REGION; ?>">
                <input type="hidden" name="query_arg" value='<?php echo json_encode($query_args); ?>'>
                <input type="hidden" name="settings" value='<?php echo json_encode($settings); ?>'>
                <div class="__fillter_filled_Select">
                    <?php foreach($filter_fields as $field) : ?>
                        <?php $this->get_filter_field($field); ?>
                    <?php endforeach; ?>
                    <div class="ToggleButton">
                        <button class="clear_cpt_filter"><?php _e('Clear', 'wp-cpt'); ?></button>
                    </div>	
                </div>	
            </form>
        </div>    
